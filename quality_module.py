...
def adjust_quality_score(self, symbol, quality_score):
    """调整质量评分以适应小幅波动市场"""
    # 获取一致性分析
    ...
    if coherence["agreement_level"] == "高度一致":
        # 高度一致性加分 - 增加调整幅度
        adjustment = min(2.5, quality_score * 0.25)  # 最多加2.5分或原分数的25%
        new_score = min(10.0, quality_score + adjustment)
        ...
        # 较强一致性加分 - 增加调整幅度
        adjustment = min(1.5, quality_score * 0.15)  # 最多加1.5分或原分数的15%
        new_score = min(10.0, quality_score + adjustment)
        adjustment_info["adjustments"].append({
            "reason": "较强时间框架一致性",
            "value": adjustment
        ...
        # 不一致减分
        adjustment = min(2.0, quality_score * 0.2)  # 最多减2分或原分数的20%
        new_score = max(0.0, quality_score - adjustment)
        adjustment_info["adjustments"].append({
            "reason": "时间框架不一致",
            "value": -adjustment
        })
        ...
        # 中等或弱一致性
        # 稍微增加中等一致性加分
        if coherence["agreement_level"] == "中等一致":
            adjustment = min(0.8, quality_score * 0.08)  # 小幅加分
            new_score = min(10.0, quality_score + adjustment)
            adjustment_info["adjustments"].append({
                "reason": "中等时间框架一致性",
                "value": adjustment
    ...
    # 环境适应性调整 - 添加新的调整因素
    if coherence["dominant_trend"] == "UP":
        # 上升趋势中，给予小幅波动更高的评分
        short_term_volatility = self.calculate_short_term_volatility(symbol)
        if short_term_volatility < 0.03:  # 低波动率
            volatility_adj = 0.8
            new_score += volatility_adj
            adjustment_info["adjustments"].append({
                "reason": "上升趋势+低波动适合小幅交易",
                "value": volatility_adj
            })
...
def calculate_short_term_volatility(self, symbol):
    """计算短期波动率，用于评估市场是否适合小幅交易"""
    try:
        # 获取15分钟数据
        tf_data = self.timeframes.get("15m", {}).get("data", {}).get(symbol)
        if tf_data is None or len(tf_data) < 10:
            return 0.05  # 默认波动率

        # 计算过去10根K线的收盘价波动率
        closes = tf_data['close'].tail(10).values
        returns = np.diff(closes) / closes[:-1]
        volatility = np.std(returns)

        return volatility
    except Exception as e:
        print_colored(f"计算波动率出错: {e}", Colors.ERROR)
        return 0.05  # 默认波动率